...
if ( ! class_exists( 'ANZU_LICENSE' ) ) {
	class ANZU_LICENSE {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '' ) {
			 ...
			 */
			$this->data_key            = 'anzu_client_' . strtolower( str_ireplace( array( ' ', '_', '&', '?', '-' ), '_', $product_id ) );
		...
		public function auto_update_message( $html, $plugin_file, $plugin_data ) {
			if ( $this->wc_am_plugin_name == $plugin_file ) {
				...
				if ( ! $this->get_api_key_status() || ! $this->get_api_key_status( true ) ) {
					return esc_html__( 'Auto-updates unavailable.', 'anzu' );
				}
				...
				if ( ! empty( $plugin_data[ 'auto-update-forced' ] ) ) {
					if ( $plugin_data[ 'auto-update-forced' ] ) {
						// Forced on.
						$text = esc_html__( 'Auto-updates enabled', 'anzu' );
					} else {
						$text = esc_html__( 'Auto-updates disabled', 'anzu' );
					}

					$action     = 'unavailable';
					$time_class = ' hidden';
				} elseif ( in_array( $plugin_file, $auto_updates, true ) ) {
					$text       = esc_html__( 'Disable auto-updates', 'anzu' );
					$action     = 'disable';
					$time_class = '';
				} else {
					$text       = esc_html__( 'Enable auto-updates', 'anzu' );
					$action     = 'enable';
					$time_class = ' hidden';
				}
		...
		public function uninstall() {
			 ...
			 * @since 2.5.1
			 *
			 * Filter anzu_client_uninstall_disable
			 * If set to false uninstall() method will be disabled.
			 */
			if ( apply_filters( 'anzu_client_uninstall_disable', true ) ) {
				global $blog_id;
		...
		public function inactive_notice() { ?>
			 ...
			 * @since 2.5.1
			 *
			 * Filter anzu_client_inactive_notice_override
			 * If set to false inactive_notice() method will be disabled.
			 */ ?>
			<?php if ( apply_filters( 'anzu_client_inactive_notice_override', true ) ) { ?>
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
                    <div class="notice notice-error">
                        <p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'anzu' ), $this->software_title, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
                    </div>
					<?php
				}
			}
		...
		public function config_page() {
			$settings_tabs = array(
				$this->wc_am_activation_tab_key   => sprintf( __( '%s', 'anzu' ), $this->wc_am_menu_tab_activation_title ),
				$this->wc_am_deactivation_tab_key => sprintf( __( '%s', 'anzu' ), $this->wc_am_menu_tab_deactivation_title )
			);
			...
			?>
            <div class='wrap'>
                <h2><?php sprintf( __( '%s', 'anzu' ), $this->wc_am_settings_title ); ?></h2>
                ...
                <form action='options.php' method='post'>
                    <div class="main">
						...
						if ( $tab == $this->wc_am_activation_tab_key ) {
							...
							submit_button( esc_html__( 'Save Changes', 'anzu' ) );
						} else {
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->wc_am_api_key_key, esc_html__( 'Key Activation', 'anzu' ), array(
				$this,
				'wc_am_api_key_text'
			), $this->wc_am_activation_tab_key );
			add_settings_field( $this->wc_am_api_key_key, esc_html__( 'License Key', 'anzu' ), array(
				$this,
				'wc_am_api_key_field'
			 ...
			 */
			if ( $this->no_product_id ) {
				add_settings_field( 'product_id', esc_html__( 'Product ID', 'anzu' ), array(
					$this,
			 ...
			 * @since 2.8
			 */ //if ( version_compare( $wp_version, '5.5', '>=' ) ) {
			//	add_settings_field( $this->wc_am_auto_update_key, esc_html__( 'Auto Plugin Updates', 'anzu' ), array(
			//		$this,
			//		'wc_am_auto_update_radio'
			//	), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
			//}

			add_settings_field( 'status', esc_html__( 'Key Status', 'anzu' ), array(
				$this,
			...
			add_settings_section( 'deactivate_button', esc_html__( 'Key Deactivation', 'anzu' ), array(
				$this,
				'wc_am_deactivate_text'
			), $this->wc_am_deactivation_tab_key );
			add_settings_field( 'deactivate_button', esc_html__( 'Deactivate Key', 'anzu' ), array(
				$this,
		...
		//	echo '<input type="radio" name="' . esc_attr( $this->wc_am_auto_update_key ) . '" value="on"' . checked( get_option( $this->wc_am_auto_update_key ), 'on', false ) . '>' . esc_html__( 'On', 'anzu' ) . '<br /><br />';
		//	echo '<input type="radio" name="' . esc_attr( $this->wc_am_auto_update_key ) . '" value="off"' . checked( get_option( $this->wc_am_auto_update_key ), 'off', false ) . '>' . esc_html__( 'Off', 'anzu' );
		//}
				...
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( ! empty( $activation_result ) ) {
						...
						if ( $activate_results[ 'success' ] === true && $activate_results[ 'activated' ] === true ) {
							add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s activated. ', 'anzu' ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ), 'updated' );
							update_option( $this->wc_am_activated_key, 'Activated' );
							update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
						}

						if ( $activate_results == false && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
							add_settings_error( 'api_key_check_text', 'api_key_check_error', esc_html__( 'Connection failed to the License Key API server. Try again later. There may be a problem on your server preventing outgoing requests, or the store is blocking your request to activate the plugin/theme.', 'anzu' ), 'error' );
							update_option( $this->wc_am_activated_key, 'Deactivated' );
						}

						if ( isset( $activate_results[ 'data' ][ 'error_code' ] ) && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
							add_settings_error( 'anzu_client_error_text', 'anzu_client_error', esc_attr( "{$activate_results['data']['error']}" ), 'error' );
							update_option( $this->wc_am_activated_key, 'Deactivated' );
						}
					} else {
						add_settings_error( 'not_activated_empty_response_text', 'not_activated_empty_response_error', esc_html__( 'The API Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', 'anzu' ), 'updated' );
					}
				...
				if ( ! empty( $deactivation_result ) ) {
					...
					if ( $activate_results[ 'success' ] === true && $activate_results[ 'deactivated' ] === true ) {
						if ( ! empty( $this->wc_am_activated_key ) ) {
							update_option( $this->wc_am_activated_key, 'Deactivated' );
							add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', esc_html__( 'API Key deactivated. ', 'anzu' ) . esc_attr( "{$activate_results['activations_remaining']}." ), 'updated' );
						}
					...
					add_settings_error( 'not_deactivated_empty_response_text', 'not_deactivated_empty_response_error', esc_html__( 'The API Key activation could not be commpleted due to an unknown error possibly on the store server The activation results were empty.', 'anzu' ), 'updated' );
				}
			}
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php esc_html__( 'Deactivates an API Key so it can be used on another blog.', 'anzu' ); ?></span>
			<?php
		}
		...
		public function activate( $args ) {
			if ( empty( $args ) ) {
				add_settings_error( 'not_activated_text', 'not_activated_error', esc_html__( 'The API Key is missing from the deactivation request.', 'anzu' ), 'updated' );
		...
		public function deactivate( $args ) {
			if ( empty( $args ) ) {
				add_settings_error( 'not_deactivated_text', 'not_deactivated_error', esc_html__( 'The API Key is missing from the deactivation request.', 'anzu' ), 'updated' );
		...
		public function update_check( $transient ) {
			...
			if ( isset( $response[ 'data' ][ 'error_code' ] ) ) {
				add_settings_error( 'anzu_client_error_text', 'anzu_client_error', "{$response['data']['error']}", 'error' );
			}
